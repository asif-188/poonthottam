...
import { subscribeToCollection, saveCashPurchase, addData, db, deleteDoc } from '../utils/storage';
...
const CashPurchase = () => {
    ...
    const sortedCashPurchases = useMemo(() => {
        return [...cashPurchases].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
    }, [cashPurchases]);

    const handleDeleteRecord = async (id) => {
        if (!window.confirm(lang === 'ta' ? 'நிச்சயமாக இந்த ரொக்கக் கொள்முதல் பதிவை நீக்க வேண்டுமா?' : 'Are you sure you want to delete this cash purchase record?')) return;
        try {
            await deleteDoc(doc(db, 'cash_purchases', id));
        ...
        const content = `
            ...
            <body onload="window.print(); window.close();">
                ...
                <div class="details">
                    ...
                    <div><span>Vendor:</span> <span>${record.vendorName || 'Cash'}</span></div>
            ...
            <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', padding: '24px' }}>
                ...
                <div style={{ overflowX: 'auto', width: '100%' }}>
                    <table style={{ width: '100%', minWidth: '600px', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>
                            {sortedCashPurchases.length === 0 ? (
                                <tr>
                                    <td colSpan={6} style={{ padding: '36px', textAlign: 'center', fontSize: '12px', color: '#94a3b8', fontStyle: 'italic' }}>
                                        {lang === 'ta' ? 'ரொக்கக் கொள்முதல் பதிவுகள் எதுவும் இல்லை.' : 'No cash purchase entries found.'}
                                    </td>
                                </tr>
                            ) : (
                                sortedCashPurchases.map((rec, idx) => {
                                    ...
                                    return (
                                        <tr key={rec.id || idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                            ...
                                            <td style={{ ...TD_S, textAlign: 'right', fontWeight: 800, color: '#d97706' }}>
